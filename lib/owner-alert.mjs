...
const PRIMARY_STORE_ID = "miami-wynwood";
const NEAREST_HELP_STORE_ID = "miami-beach";

function planFulfillment(qty, primaryCap, helpCap) {
  ...
  return {
    ...
    primaryShare,
    helpShare,
    uncovered: total - primaryShare - helpShare,
  ...
  if (command === "APPROVE") {
    ...
    await sql`
      INSERT INTO store_events (
        ...
        ${`Case ${caseId}: Wynwood ${plan.primaryShare}${
          plan.needsHelp ? ` + Miami Beach ${plan.helpShare}` : ""
        } pies.`},
    ...
    if (plan.needsHelp) {
      await sql`
        INSERT INTO store_events (
          ...
          'fulfillment_assist',
          'watch',
          'Owner approved — help Wynwood',
          ${`Produce ${plan.helpShare} pies for case ${caseId}.`},
    ...
    replyText = plan.needsHelp
      ? `Approved. Coordinating now: Wynwood ${plan.primaryShare} + Miami Beach ${plan.helpShare}. Managers notified. Case ${caseId}.`
      : `Approved. Wynwood covering ${plan.primaryShare} pies solo. Case ${caseId}.`;
  } else if (command === "REVIEW") {
    status = "review";
    const breach = prior.breachSummary || "unusual kitchen/capacity load";
    const qty = prior.order?.qty || "—";
    const where = prior.order?.where || "—";
    replyText = [
      `Review ${caseId}:`,
      `${prior.order ? `${qty} pies · ${where}` : "Live SPC alert"}`,
    ...
    try {
      dial = await placeOwnerCall({
        ...
        storeName:
          prior.storeName ||
          (storeId === PRIMARY_STORE_ID ? "Joe's Pizza Miami Wynwood" : storeId),
        caseId,
        flags: prior.flags || [],
