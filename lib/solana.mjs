...
/** 64-byte ed25519 secret without loading @solana/web3.js (Vercel-safe). */
async function loadSecretKeyBytes() {
  const secret = await parseSecretKey(process.env.SOLANA_SECRET_KEY);
  if (secret.length !== 64) {
    throw new Error(`SOLANA_SECRET_KEY must be 64 bytes (got ${secret.length})`);
  }
  return secret;
}
...
export async function loadKeypair() {
  ...
  const secret = await loadSecretKeyBytes();
...
export async function getWalletBalanceLamports() {
  ...
  try {
    ...
    console.warn("[solana] balance check failed:", err?.message || err);
    return 0;
  }
}

/**
 * Off-chain ed25519 attestation of a payload hash (no SOL / no sandbox needed).
 * Used when the faucet/RPC airdrop is unavailable; upgrade to memo tx later.
 */
export async function signHashOffChain(hashHex) {
  if (!solanaConfigured()) {
    throw new Error("SOLANA_SECRET_KEY not configured");
  }
  // Pure ed25519 path — do not import @solana/web3.js (breaks on Vercel ESM).
  const bs58 = await loadBs58();
  const nacl = (await import("tweetnacl")).default;
  const secretKey = await loadSecretKeyBytes();
  const publicKey = secretKey.slice(32);
  const message = Buffer.from(`ownerradar:v1:${hashHex}`, "utf8");
