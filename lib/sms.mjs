...
export function buildOwnerAlertSms({
  ...
  const shop = storeName || "a business in your portfolio";
  const plain = plainBreach(breachSummary);
  const orderBit = order?.qty
    ? ` After a ${order.qty}-panel commission${
        order.where ? ` for ${order.where}` : ""
      }.`
    : ".";
  const moneyBit =
  ...
  const helpBit =
    plan?.needsHelp && plan.helpShare
      ? ` Closest help: Pollinator can take ~${plan.helpShare} panels.`
      : "";

  const lines = [
    `OwnerRadar: ${shop} looks off vs your other businesses${moneyBit}.`,
...
function plainBreach(summary) {
  ...
  if (!cleaned || cleaned.length < 4) {
    return "Ops load / capacity is way outside what's normal for this business.";
  }
