...
function buildTwilioSay({ storeName, breachSummary, order, caseId }) {
  const shop = storeName || "one of your businesses";
  const plain = String(breachSummary || "unusual ops load")
    .replace(/[±+]?\d+(\.\d+)?\s*σ/gi, "")
    .replace(/;/g, ",")
    .trim();
  const orderBit = order?.qty
    ? ` after a ${order.qty} panel commission${order.where ? ` for ${order.where}` : ""}`
    : "";
  return `OwnerRadar here. ${shop} looks off versus your other businesses${orderBit}. ${plain || "Capacity is outside normal bands."} Case ${caseId}.`;
}
