...
/** Outbound owner voice dials — off unless OWNER_CALLS_ENABLED=true. */
export function ownerCallsEnabled() {
  const v = process.env.OWNER_CALLS_ENABLED?.trim().toLowerCase();
...
export async function placeOwnerCall({
  ...
  if (!ownerCallsEnabled()) {
    const err = new Error(
      "Owner calls disabled — set OWNER_CALLS_ENABLED=true to re-enable"
    );
    err.code = "OWNER_CALLS_DISABLED";
    throw err;
  }
  ...
  if (process.env.RETELL_API_KEY && process.env.RETELL_FROM_NUMBER) {
    ...
    // Fallback: Twilio voice with a short spoken brief (no Retell/ElevenLabs needed)
    provider = "twilio";
    ...
    const call = await client.calls.create({
      to,
      from,
      twiml: `<Response><Say voice="Polly.Joanna">${escapeXml(say)}</Say><Pause length="1"/><Say voice="Polly.Joanna">Reply APPROVE by text to coordinate, or hang up.</Say></Response>`,
    });
    providerResponse = { call_id: call.sid, sid: call.sid, status: call.status };
  } else {
    const err = new Error(
      "No dial provider configured. Set RETELL_*, ELEVENLABS_*, or TWILIO_ACCOUNT_SID + TWILIO_AUTH_TOKEN + TWILIO_FROM_NUMBER."
    );
...
export async function dialOwnerForOutOfControl({
  ...
  if (!ownerCallsEnabled()) {
    return {
      ...
      skipped: [
        {
          reason: "owner_calls_disabled",
          message:
            "Owner calls disabled — set OWNER_CALLS_ENABLED=true to re-enable",
        },
      ],
      analysis: { alertZ: ALERT_Z, outOfControlCount: 0, enabled: false },
    };
  }
...
function buildTwilioSay({ storeName, breachSummary, order, caseId }) {
  const shop = storeName || "one of your Joe's shops";
  const plain = String(breachSummary || "unusual kitchen load")
    .replace(/[±+]?\d+(\.\d+)?\s*σ/gi, "")
    .replace(/;/g, ",")
    .trim();
  const orderBit = order?.qty
    ? ` after a ${order.qty} pie catering order${order.where ? ` to ${order.where}` : ""}`
    : "";
  return `OwnerRadar here. ${shop} looks off versus your other shops${orderBit}. ${plain || "Capacity is outside normal bands."} Case ${caseId}.`;
}
