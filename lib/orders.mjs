...
/** Voice orders land on Plant The Future; Pollinator is nearest sister help. */
export const PRIMARY_STORE_ID = "plant-the-future";
export const NEAREST_HELP_STORE_ID = "pollinator";

/** Owner is always dialed when panel/piece count exceeds this (exclusive). */
export const OWNER_CALL_QTY_THRESHOLD = 8;

const DEFAULT_UNIT_CENTS = 75000; // ~$750 / moss wall panel
...
async function drawInventory(sql, storeId, pizzaCount, caseId, reasonSuffix = "") {
  ...
  for (const [sku, perUnit] of [
    ...
    const delta = -(pizzaCount * perUnit);
  ...
  await sql`
    INSERT INTO utility_readings (
      ...
      ${storeId},
      ${pizzaCount * 0.4},
      ${Math.max(0.2, pizzaCount * 0.01)},
      ${Math.max(1, pizzaCount * 0.35)},
      ${pizzaCount * 8.5}
    )
  `;
}
...
/**
 * Build a fulfillment split: Plant The Future takes up to local capacity;
 * Pollinator covers the rest when needed.
 */
...
/**
 * Insert a commission/phone order. Always attributed to Plant The Future.
 * Owner escalation when:
 *   - SPC ≥2σ out of control, OR
 *   - panel count > OWNER_CALL_QTY_THRESHOLD (8)
 */
export async function insertCateringOrder({
  ...
  const pizzaType = String(item || "moss wall panels");
  ...
  await drawInventory(
    ...
    resolvedCaseId,
    plan.needsHelp ? "gallery_share" : "full"
  );

  if (plan.needsHelp && plan.helpShare > 0) {
    await drawInventory(sql, helpId, plan.helpShare, resolvedCaseId, "workshop_help");
    ...
    await sql`
      INSERT INTO store_events (
        ...
        'fulfillment_assist',
        'watch',
        'Helping Plant The Future fulfill',
        ${`Pollinator producing ${plan.helpShare} panels to help Plant The Future cover ${pizzaCount}-panel order to ${where}.`},
  ...
  const breachSummary = largeCatering
    ? spcOutOfControl
      ? `${pizzaCount}-panel order (over ${OWNER_CALL_QTY_THRESHOLD}) · ${spcBreach}`
      : `${pizzaCount}-panel commission (over ${OWNER_CALL_QTY_THRESHOLD})`
    : spcBreach;

  const fulfillCopy = plan.needsHelp
    ? ` Fulfillment: Plant The Future ${plan.primaryShare} + Pollinator ${plan.helpShare}${plan.uncovered ? ` (${plan.uncovered} still uncovered)` : ""}.`
    : " Fulfillment: Plant The Future solo.";

  const eventTitle = largeCatering
    ? `Large commission · ${pizzaCount} panels`
    : spcOutOfControl
      ? `Out of control ≥${ALERT_Z}σ · ${pizzaCount}-panel order`
      : "Order accepted";
  const eventBody = outOfControl
    ? `Gallery accepted ${bodyNote}.${fulfillCopy}${
        spcOutOfControl ? ` SPC: ${spcBreach}.` : ""
      }${largeCatering ? ` Auto-escalating: qty > ${OWNER_CALL_QTY_THRESHOLD}.` : ""}`
    : `Gallery accepted ${bodyNote}.${fulfillCopy} KPIs remain within ${ALERT_Z}σ.`;
  ...
  if (dialOwner && outOfControl) {
    ...
    // qty > threshold always forces a dial (bypasses SPC-only filter + cooldown gate).
    try {
      if (largeCatering) {
      ...
      if (largeCatering && twilioConfigured() && ownerPhone()) {
        ...
        await sql`
          INSERT INTO store_events (
            ...
            'owner_sms',
            'alert',
            'OwnerRadar texted owner (large commission)',
            ${body},
            ${JSON.stringify({
              caseId: resolvedCaseId,
              sid: msg.sid,
